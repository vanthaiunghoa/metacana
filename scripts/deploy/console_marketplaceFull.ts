...
async function main(): Promise<void> {
  ...
  const marketplace: Contract = await Marketplace_Factory.deploy(protocolControl.address, forwarder.address, marketContractURI);

  console.log(`Deploying Marketplace: ${marketplace.address} at tx hash: ${marketplace.deployTransaction.hash}`);

  await marketplace.deployed();

  const addModuleTxMarket = await protocolControl.addModule(marketplace.address, ModuleType.Marketplace);
