...
export function fixSignature (signature) {
...
export function toEthSignedMessageHash (messageHex) {
 ...
 */
export const getSignFor = (contract, signer) => (redeemer, methodName, methodArgs = []) => {
  const parts = [
    contract.address,
    redeemer,
  ];
  ...
  if (methodName) {
    if (methodArgs.length > 0) {
      parts.push(
        contract.contract.methods[methodName](...methodArgs.concat([DUMMY_SIGNATURE] as any)).encodeABI()
  ...
  return web3.eth.sign(messageHex as any, signer);
};
