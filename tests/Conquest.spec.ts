...
describe('Conquest', () => {
  ...
  beforeEach(async () => {
    ...
    await silverFactory.assignOwnership(factoryContract.address, 1)
    await goldFactory.assignOwnership(factoryContract.address, 1)

    // Assing vars
    factory = factoryContract.address

    // Activate gold and silver factories and authorize them
    await MetacanaAssetsContract.activateFactory(silverFactory.address);
    await MetacanaAssetsContract.activateFactory(goldFactory.address);
    await MetacanaAssetsContract.addMintPermission(silverFactory.address, silver_minRange, silver_maxRange, startTime, endTime);
    await MetacanaAssetsContract.addMintPermission(goldFactory.address, gold_minRange, gold_maxRange, startTime, endTime);
  ...
  describe('Getter functions', () => {
    describe('metacanaAssets() function', () => {
  ...
  describe('Conquest', () => {
    ...
    context('Using safeBatchTransferFrom', () => {
      it('should PASS if caller sends 100 ticket', async () => {
        const tx = userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID], [amount], [], TX_PARAM)
      ...
      it('should REVERT if ids length is more than 1', async () => {
        const tx = userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID, ticketID], [amount], [], TX_PARAM)
      ...
      it('should REVERT if amounts length is more than 1', async () => {
        const tx = userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID], [amount, amount], [], TX_PARAM)
        await expect(tx).to.be.rejectedWith(RevertError("ERC1155PackedBalance#_safeBatchTransferFrom: INVALID_ARRAYS_LENGTH"))
      })
      ...
      it('should REVERT if sent more than 1 ticket', async () => {
        ...
        const tx = userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID], [invalid_amount], [], TX_PARAM)
      ...
      it('should REVERT if caller is already in a conquest', async () => {
        ...
        const tx2 = userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID], [amount], [], TX_PARAM)
      ...
      it.skip('should REVERT if caller tries to do conquest faster than limit', async () => {
        await userMetacanaAssetsContract.safeBatchTransferFrom(userAddress, factory, [ticketID], [amount], [], TX_PARAM)
    ...
    context('Using safeTransferFrom', () => {
      it('should PASS if caller sends 100 ticket', async () => {
        const tx = userMetacanaAssetsContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
        await expect(tx).to.be.fulfilled
      })
      ...
      it('should REVERT if sent more than 1 ticket', async () => {
        ...
        const tx = userMetacanaAssetsContract.safeTransferFrom(userAddress, factory, ticketID, invalid_amount, [], TX_PARAM)
        await expect(tx).to.be.rejectedWith(RevertError("Conquest#entry: INVALID_ENTRY_TOKEN_AMOUNT"))
      })

      it('should REVERT if caller is already in a conquest', async () => {
        await userMetacanaAssetsContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
        const tx = userMetacanaAssetsContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
    ...
    context('When ticket was sent to factory', () => {
      let tx;
      beforeEach(async () => {
        tx = await userMetacanaAssetsContract.safeTransferFrom(userAddress, factory, ticketID, amount, [], TX_PARAM)
      })

      it('should burn entry sent balance', async () => {
        let factory_balance = await MetacanaAssetsContract.balanceOf(factory, ticketID)
        expect(factory_balance).to.be.eql(constants.Zero)
      })

      it('should update user ticket balance', async () => {
        let user_balance = await MetacanaAssetsContract.balanceOf(userAddress, ticketID)
        expect(user_balance).to.be.eql(ticketAmount.sub(amount))
      })

      it('should set user as being in conquest', async () => {
        let value = await factoryContract.isActiveConquest(userAddress)
        expect(value).to.be.eql(true)
      })

      it('should update user conquest count', async () => {
        let value = await factoryContract.conquestsEntered(userAddress)
      ...
      it('should update next conquest time for user', async () => {
        ...
        let value = await factoryContract.nextConquestTime(userAddress)
        expect(value).to.be.eql(BigNumber.from(timeStamp).add(DELAY))
      })
      ...
      it('should emit ConquestEntered event', async () => {
        ...
        expect(logs[0].topics[0]).to.be.eql(factoryContract.interface.getEventTopic(factoryContract.interface.events["ConquestEntered(address,uint256)"]))
      })
      ...
      describe('ConquestEntered Event', () => {
        ...
        beforeEach(async () => {
          ...
          args = factoryContract.interface.decodeEventLog(factoryContract.interface.events["ConquestEntered(address,uint256)"],logs[0].data, logs[0].topics)
        })
  ...
  describe('exitConquest()', () => {
    ...
    it('should REVERT if caller is not in conquest', async () => {
      const tx = factoryContract.exitConquest(userAddress, silver_ids, gold_ids)
      ...
      conditions.forEach(function(condition) {
        context(condition[2] as string, () => {
          ...
          it('should PASS if caller is owner', async () => {
            const tx = factoryContract.exitConquest(userAddress, silverRewardIds, goldRewardIds)
            await expect(tx).to.be.fulfilled
          })

          it('should PASS if rewards are duplicates', async () => {
            const tx = factoryContract.exitConquest(userAddress, [23, 23], [])
            await expect(tx).to.be.fulfilled
          })

          it('should PASS if rewards are unsorted', async () => {
            const tx = factoryContract.exitConquest(userAddress, [7, 1], [])
            await expect(tx).to.be.fulfilled
          })
          ...
          it('should REVERT if caller is not owner', async () => {
            const tx = userFactoryContract.exitConquest(userAddress, silverRewardIds, goldRewardIds)
          ...
          it('should REVERT if trying to mint more than possible amount', async () => {
            const tx4 = factoryContract.exitConquest(userAddress, [1, 2, 3], [])
            await expect(tx4).to.be.rejectedWith(RevertError("Conquest#exitConquest: INVALID_REWARDS"))

            const tx1 = factoryContract.exitConquest(userAddress, [1], [GOLD_SPACE.add(1), GOLD_SPACE.add(2)])
            await expect(tx1).to.be.rejectedWith(RevertError("Conquest#exitConquest: INVALID_REWARDS"))

            const tx2 = factoryContract.exitConquest(userAddress, [1, 2], [GOLD_SPACE.add(1)])
            await expect(tx2).to.be.rejectedWith(RevertError("Conquest#exitConquest: INVALID_REWARDS"))

            const tx3 = factoryContract.exitConquest(userAddress, [], [GOLD_SPACE.add(1)])
            await expect(tx3).to.be.rejectedWith(RevertError("Conquest#exitConquest: INVALID_REWARDS"))

            const tx5 = factoryContract.exitConquest(userAddress, [1], [GOLD_SPACE.add(1)])
            await expect(tx5).to.be.fulfilled
          })
      
          context('When exitConquest was called', () => {
            beforeEach(async () => {
              await factoryContract.exitConquest(userAddress, silverRewardIds, goldRewardIds)
            })
            ...
            it('should update user silver cards balance', async () => {
              ...
              let userBalances = await userMetacanaAssetsContract.balanceOfBatch(user_addresses, silver_ids)
            ...
            it('should update user gold cards balance', async () => {
              ...
              let user_addresses = new Array(n_ids).fill('').map((a, i) => userAddress)
              let userBalances = await userMetacanaAssetsContract.balanceOfBatch(user_addresses, gold_ids)
              for (let i = 0; i < n_ids; i++) {
