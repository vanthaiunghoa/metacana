...
describe('ConquestEntriesFactory', () => {
  ...
  beforeEach(async () => {
    ...
    await metacanaAssetsContract.addMintPermission(factory, ticketID, ticketID, startTime, endTime);

    // Let owner be a "factory" to mint silver cards to test conquest
    await metacanaAssetsContract.activateFactory(ownerAddress);
    await metacanaAssetsContract.addMintPermission(ownerAddress, silverMinRange, silverMaxRange.add(100), startTime, endTime);
    ...
    conditions.forEach(function(condition) { 
      context(condition as string, () => {
        ...
        it('should PASS if caller sends assets', async () => {
          ...
          if (condition == conditions[0]) {
            for (let i = 0; i < ids.length; i++) {
              tx = userMetacanaAssetContract.safeTransferFrom(userAddress, factory, ids[i], amounts[i], [], TX_PARAM)
            }
          } else if (condition == conditions[1]) {
            tx = userMetacanaAssetContract.safeBatchTransferFrom(userAddress, factory, ids, amounts, [], TX_PARAM)
        ...
        it('should PASS if receiver is specified', async () => {
          ...
          if (condition == conditions[0]) {
            for (let i = 0; i < ids.length; i++) {
              tx = userMetacanaAssetContract.safeTransferFrom(userAddress, factory, ids[i], amounts[i], encoded_recipient, TX_PARAM)
            }
          } else if (condition == conditions[1]) {
            tx = userMetacanaAssetContract.safeBatchTransferFrom(userAddress, factory, ids, amounts, encoded_recipient, TX_PARAM)
          } else if (condition == conditions[2]) {
        ...
        it('should REVERT if asset is not silver card or wDAI', async () => {
          let tx;
          await metacanaAssetsContract.batchMint(userAddress, [silverMaxRange.add(1), silverMaxRange.add(2)], [nTokensPerType, nTokensPerType] , [])
          if (condition == conditions[0]) {
            tx = userMetacanaAssetContract.safeTransferFrom(userAddress, factory, silverMaxRange.add(1), nTokensPerType, [])
          } else {
            tx = userMetacanaAssetContract.safeBatchTransferFrom(userAddress, factory, [silverMaxRange.add(1), silverMaxRange.add(2)], [nTokensPerType, nTokensPerType], [])
          }
          ...
          recipientConditions.forEach(function(recipient_cond) { 
            context(recipient_cond as string, () => {
              ...
              beforeEach(async () => {
                ...
                if (condition == conditions[0]) {
                  for (let i = 0; i < ids.length; i++) {
                    tx = await userMetacanaAssetContract.safeTransferFrom(userAddress, factory, ids[i], amounts[i], data, TX_PARAM)
                  }
                } else if (condition == conditions[1]) {
                  tx = await userMetacanaAssetContract.safeBatchTransferFrom(userAddress, factory, ids, amounts, data, TX_PARAM)
                } else if (condition == conditions[2]) {
              ...
              if (condition == conditions[0] || condition == conditions[1]) {
                it('should leave factory silver cards balance to 0', async () => {
                  ...
                  let factory_balances = await userMetacanaAssetContract.balanceOfBatch(factory_addresses, ids)
                  for (let i = 0; i < ids.length; i++) {
                ...
                it('should update users silver cards balance', async () => {
                  ...
                  let userBalances = await userMetacanaAssetContract.balanceOfBatch(user_addresses, ids)
                  for (let i = 0; i < ids.length; i++) {
                ...
                it('should update recipient conquest entries balance', async () => {
                  let userBalance = await userMetacanaAssetContract.balanceOf(recipient, ticketID)
              ...
              it('should not increase entry balance of factory', async () => {
                let factory_balance = await userMetacanaAssetContract.balanceOf(factory, ticketID)
