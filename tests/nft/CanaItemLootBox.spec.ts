...
import * as setup from "../../scripts/lib/setupCreatureAccessories"
import * as vals from "../../scripts/lib/valuesCommon"
import * as testVals from "../../scripts/lib/testValuesCommon";
...
describe("CanaItemLootBox", () => {
  ...
  before(async () => {
    ...
    await setup.setupPreMintBox(canaItemLootBox, ownerAddress);
  ...
  describe('#mint()', () => {
    it('should work for owner()', async () => {
      const option = vals.LOOTBOX_OPTION_META;
      const amount = 1;
  ...
  describe('#unpack()', () => {
    it('should mint guaranteed class amounts for each option', async () => {
      for (let i = 0; i < vals.NUM_LOOTBOX_OPTIONS; i++) {
        ...
        const balance = await canaItemLootBox.balanceOf(ownerAddress, option);
        await canaItemLootBox.safeTransferFrom(
          ownerAddress,
          userAddress,
          option,
          amount,
