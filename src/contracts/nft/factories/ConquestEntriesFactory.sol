...
contract ConquestEntriesFactory is IERC1155TokenReceiver, TieredOwnable {
  ...
  /**
   * @notice Create factory, link metacana assets and store initial parameters
   * @param _firstOwner             Address of the first owner
   * @param _metacanaAssetsAddress The address of the ERC-1155 Assets Token contract
  ...
  constructor(
    address _firstOwner,
    address _metacanaAssetsAddress,
  ...
  {
    require(
      _metacanaAssetsAddress != address(0) && 
    ...
    // Assets
    metacanaAssets = IMetacanaAssets(_metacanaAssetsAddress);
  ...
  { 
    ...
    if (msg.sender == address(metacanaAssets)) {
      ...
      metacanaAssets.batchBurn(_ids, _amounts);
    ...
    metacanaAssets.mint(receiver, conquestEntryID, n_entries*10**ENTRIES_DECIMALS, "");
