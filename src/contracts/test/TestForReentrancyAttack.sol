...
import "../nft/CanaItemFactory.sol";
...
contract TestForReentrancyAttack is IERC1155Receiver {
    ...
    /*function attack(uint256 _totalToMint) external {
        ...
        CanaItemFactory(factoryAddress).mint(1, address(this), 1, "");
        }*/

    // We attempt a reentrancy attack here by recursively calling the
    // CanaItemFactory that created the CreatureAccessory ERC1155 token
    // that we are receiving here.
    // We expect this to fail if the CanaItemFactory.mint() function
        ...
        returns(bytes4)
    {
        ...
        if(balance < totalToMint)
        {
            ...
            CanaItemFactory(factoryAddress)
                .mint(address(this),1, 1, "");
        }
